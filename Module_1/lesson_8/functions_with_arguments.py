...
def make_short_url(long_url):
    print(f"Access to some_url: {some_url}")  # Exists in main program so we access from functions

    print("\tSTART OF THE FUNCTION")
    print(f"Get {long_url} as original URL")
    if not isinstance(long_url, str):
        print("Wrong URL Type, Str was expected")
        long_url = str(long_url)

    if long_url.startswith("http"):
        short_url = long_url.split("/")[2]
...
print(some_url)
make_short_url(some_url)
# print(long_url)  # Exists only inside function
